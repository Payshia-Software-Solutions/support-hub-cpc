            ...
            {isLoadingBalance ? <Skeleton className="h-40"/> : balanceData && (
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4 text-center">
                        <Card><CardHeader><CardTitle>Total Paid</CardTitle></CardHeader><CardContent><p className="text-xl font-bold">LKR {balanceData.totalPaymentAmount.toLocaleString()}</p></CardContent></Card>
                        <Card><CardHeader><CardTitle>Outstanding</CardTitle></CardHeader><CardContent><p className="text-xl font-bold text-destructive">LKR {balanceData.studentBalance.toLocaleString()}</p></CardContent></Card>
                    </div>
                    <Card><CardHeader><CardTitle className="text-base">Recent Payments</CardTitle></CardHeader><CardContent>
                        <ul className="space-y-2">
                        {Object.values(balanceData.paymentRecords).slice(0, 5).map(rec => (
                            <li key={rec.id} className="flex justify-between text-sm">
                                <span>{rec.course_code} - {new Date(rec.paid_date).toLocaleDateString()}</span>
                                <span className="font-semibold">{parseFloat(rec.paid_amount).toLocaleString()}</span>
                            </li>
                        ))}
                        </ul>
                    </CardContent></Card>
...
function UserDetailsSection({ request }: { request: PaymentRequest }) {
    const { data: userData, isLoading: isLoadingUser, isError: isUserError, error: userError } = useQuery({
    ...
    const { data: previousPayments, isLoading: isLoadingPayments, isError: isPaymentsError } = useQuery({
        queryKey: ['previousPayments', request.unique_number],
        queryFn: () => getPaymentRequestsByReference(request.unique_number),
        enabled: !!request,
        retry: false,
    });
    ...
    if (isUserError) {
        return <Alert variant="destructive"><AlertCircle className="h-4 w-4" /><AlertTitle>Error Fetching User</AlertTitle><AlertDescription>{(userError as Error).message}</AlertDescription></Alert>;
    }
    if (!userData) {
        return <Alert><AlertDescription>No user details available for this request type.</AlertDescription></Alert>;
    }

    const isTempUser = 'email_address' in userData;

    return (
        <div className="space-y-4">
            <div className="rounded-md border p-4 bg-muted/50">
                <h3 className="font-semibold text-base mb-3">User Information</h3>
                {isTempUser ? (
             ...
             <Card>
                ...
                <CardContent>
                    {isLoadingPayments ? <Skeleton className="h-20 w-full" /> : (
                        (previousPayments && previousPayments.length > 0) ? (
                            <div className="space-y-2">
                                {previousPayments.map(p => (
                                    <div key={p.id} className="text-xs p-2 border rounded-md flex justify-between items-center">
                                        <div>
                                            <p className="font-mono">ID: {p.id}</p>
                                            <p><strong>Amount:</strong> {p.paid_amount}</p>
                                            <p><strong>Date:</strong> {format(new Date(p.created_at), 'Pp')}</p>
                                        </div>
                                         <Badge variant={p.payment_status === 'Pending' ? 'destructive' : p.payment_status === 'Approved' ? 'default' : 'secondary'}>
                                            {p.payment_status}
                                        </Badge>
                                    </div>
                                ))}
                            ...
                            <p className="text-sm text-muted-foreground text-center">No previous submissions found.</p>
                        )
                    )}
                </CardContent>
            </Card>
